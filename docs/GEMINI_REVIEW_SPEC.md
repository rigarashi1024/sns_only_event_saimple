...
通常の人間向けコメントは Gemini の入力にも再実行トリガーにも使いません。Gemini に渡したい確認メモは、専用 marker と `/gemini-review` を含むコメントとして投稿します。
...
4. GitHub API から PR コメント一覧を取得する
5. `<!-- gemini-review-context -->` marker があるコメントだけを Gemini 用 context として抽出する
6. レビュー対象外のファイルを除外する
7. 各ファイルごとに patch と Gemini 用 context を Gemini に渡してレビューする
8. 結果を集約して PR コメントとして投稿する
9. 既存の Gemini コメントは編集せず、新しいコメントとして追加する

既存コメントを更新しないのは意図した仕様です。レビューコメントを上書きすると、どの push でどの指摘が出たのか、また人間がどの時点でどう判断したのかが追いにくくなるためです。
...
- 最新の Gemini レビュー結果は、同じ PR 内で最も新しい `### 🤖 Gemini PR Review` コメントとして識別する
- 過去コメントの指摘が後続コミットで解消された場合でも、過去コメントは当時の判断履歴として残す

この運用ではコメント数が増えるデメリットがあります。一方で、このリポジトリではレビューの流れと判断理由の追跡性を優先するため、コメント集約よりも時系列の保存を重視します。
...
- 人間向けの確認コメントと Gemini に渡す確認メモを分ける
- Gemini に渡す確認メモは `<!-- gemini-review-context -->` と `/gemini-review` を含め、通常コメントには含めない
